...
{   
    ...
    function play(song)
    {
        ...
        if (ytdl.validateURL(song))
        {
            yts({videoId: ytdl.getURLVideoID(song)}).then( (result) =>
            {
                ...
                vc.join().then(ss => ss.play(stream, { seek: 0, volume: 1 }).on('finish', () => { message.channel.send('Your song has been completed'); pog.clearNow(message.guild.id); }));
    ...
    function qplay()
    {
        ...
        if (link.length !== 0)
        {
            ...
            else { vc.join().then(ss => ss.dispatcher.end()); pog.counter.clear(message.guild.id); pog.changeNow(message.guild.id, false); pog.clear(message.guild.id); message.channel.send('🎵 Your song has been completed 🎵 / 🎵 Queue ended 🎵 / 🎵 Queue cleared 🎵 '); }
        }
        else { message.channel.send('There are no songs in queue'); pog.counter.clear(message.guild.id); pog.changeNow(message.guild.id, false); }
    };
        ...
        if (one)
        {
            if (one.toLowerCase() === 'play')
            {
                if (vc)
                {
                    if (two)
                    {
                        ...
                        {
                            message.channel.send('__🎵 Some songs are already in the queue 🎵__\r\n\r\nSend **`YES`** if u want to clear the q\r\nSend **`NO`** if u dont want to clear the q\r\n\r\nIf you dont send a message within **30 secs**, Queue will be cleared automatically\r\nYou__can send any msgs__ within that **30 sec**, but if you send **`YES`** or **`NO`** the bot will clear the queue depending on the message');
                            ...
                            collector.on('end', (xyz) =>
                            {
                                ...
                                else if (response.length > 1)
                                {
                                    ...
                                    else { message.channel.send('🎵Queue cleared automatically🎵'); }
                                }
                            });
                        }
                    }
    
                    else { message.channel.send('You need to give a **Youtube link** or **Song name**'); }
                }

                else { message.channel.send('🎵 You need to join a voice channel before you can use that command 🎵'); }
            }
            ...
            else if (one.toLowerCase() === 'q')
            {
                ...
                else
                {
                    ...
                    else { message.channel.send('There is no song in queue'); }
                }
            }
            ...
            else if (one.toLowerCase() === 'ping')
            {
                ...
                .then((msg) =>
                {
                    ...
                    msg.edit(ping_embed); setTimeout(() => { msg.edit(new_ping_embed); }, 1000 * 2);
                }
                );
            }
            ...
            else if (one.toLowerCase() === 'search')
            {
                ...
                else { message.channel.send('You need to give the name of song **(Not a emoji)**'); }
            }
